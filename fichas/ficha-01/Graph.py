...
class Graph:
    ...
    def procura_DFS(self, source, destiny, path, visited):
        visited.add(source)
        path.append(source)
        ...
        for (node, w) in self.m_graph[source]:
            # nodo não visitado
            if node not in visited:
                c, f = self.procura_DFS(node, destiny, path, visited)
                if f:
                    return w + c, True
    ...
    def procura_BFS(self, source, destiny, path, visited):
        visited.add(source)
        ...
        while len(queue) != 0 and not found:
            temp = queue.pop(0)
            for (node, w) in self.m_graph[temp]:
                # nodo não visitado
                if node not in visited:
                    visited.add(node)
                    parents[node] = temp
                    costs[node] = costs[temp] + w
                    if node == destiny:
                        found = True
                        break
                    queue.append(node)
        ...
        if found:
            temp = destiny
            while parents[temp] is not None:
                path.append(temp)
                temp = parents[temp]
            path.append(source)
            path.reverse()
